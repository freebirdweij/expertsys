 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/expfetch/acptredraw")
public class RedrawAcceptController extends BaseController {
	...
	public String directdrawunit(ProjectExpert projectExpert, HttpServletRequest request, HttpServletResponse response, Model model, RedirectAttributes redirectAttributes) {
		...
		if(techIdsNo==null||techIdsNo.equals("")){
			for(ProjectInfo prj:pis){
				reslist.addAll(projectExpertService.findReviewingExpertByProject(new Page<ExpertConfirm>(),prj.getId()).getList());
			}
		...
		if(discIds!=null&&!discIds.equals("")){
			for(String did:dids){
				...
				for(ProjectInfo pi:pis){
					projectExpertService.updateProjectExpertAbsence(Constants.Fetch_Accept_Sussess, di[1], pi.getId(), di[0]);
					//更新评审时间
					projectExpertService.updateProjectExpertReviewDate(Constants.Fetch_Accept_Sussess,pi.getId(),projectExpert.getReviewBegin(),projectExpert.getReviewEnd());
				}
			}
		}
        ...
        for(Office ec : tlist){
        	ExpertConfirm ecn = projectExpertService.findAExpertByUnitAndKindRemoveSomeExperts(ec, Constants.Expert_Kind_Technical,techIdsNo);
        	if(ecn==null){
        ...
        for(Office ec : elist){
        	ExpertConfirm ecn = projectExpertService.findAExpertByUnitAndKindRemoveSomeExperts(ec, Constants.Expert_Kind_Economic,techIdsNo);
        	if(ecn==null){
    			addMessage(model, "本次抽取失败，请重抽！");
    	        projectExpert.setReviewBegin(new Timestamp(projectExpert.getReviewBegin().getTime()));
    	        projectExpert.setReviewEnd(new Timestamp(projectExpert.getReviewEnd().getTime()));
	    ...
	    for (ExpertConfirm ec : erclist) {
	    	...
	    	pExpert.setFetchStatus(Constants.Fetch_AcceptRedraw_Failure);
			...
			strb.append(Constants.Log_Function_FetchAcceptRedraw).append("验收前补抽专家,").append(Constants.Log_Expert_Name).append(ec.getExpertInfo().getName()).append(",")
