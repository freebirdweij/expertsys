...
import com.freebirdweij.cloudroom.common.utils.excel.ExportFetchExcel;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/expfetch/fetchsearch")
public class FetchSearchController extends BaseController {
	...
	public String reviewinglist(ProjectExpert projectExpert, HttpServletRequest request, HttpServletResponse response, Model model) {
		User user = UserUtils.getUser();
		/*if (!user.isAdmin()){
			projectExpert.setCreateBy(user);
		}*/
		...
		//String status = "'"+Constants.Fetch_Review_Sussess+"','"+Constants.Fetch_Accept_Sussess+"','"+Constants.Fetch_Accepted_Sussess+"'";
		String[] status = {Constants.Fetch_Review_Sussess,Constants.Fetch_Accept_Sussess,Constants.Fetch_Accepted_Sussess};
        Page<Object> page = projectExpertService.findProjectExpertFetchByUnitAndStatus(new Page<Object>(request, response),projectExpert, user.getCompany().getId(),status); 
        model.addAttribute("page", page);
		return "modules/expfetch/fetchsearch/reviewingList";
	}
	...
	public String unitmethod(ProjectExpert projectExpert, Model model, HttpServletRequest request, HttpServletResponse response,@RequestParam("fetchTime") Integer fetchTime,@RequestParam("prjid") String prjid,@RequestParam("fetchStatus") String fetchStatus) {
		String[] status = {fetchStatus};
		List<ProjectInfo> plist = projectExpertService.findProjectExpertByFetchAndStatus(new Page<ProjectExpert>(request, response), fetchTime, status);
        model.addAttribute("plist", plist);
        
		ProjectExpert pExpert = projectExpertService.findProjectExpertByPrjAndStatus(prjid,fetchStatus);
		projectExpert.setPrjid(prjid);
		...
		List<ProjectExpert> olist = null;
		if(fetchStatus.equalsIgnoreCase(Constants.Fetch_Review_Sussess)){
		String[] stas = {Constants.Fetch_Review_Sussess,Constants.Fetch_ReviewRedraw_Sussess};
		olist = projectExpertService.findMutiProjectExpertByPrjAndStatusOntime(prjid,stas);
		model.addAttribute("olist", olist);
		}
		if(fetchStatus.equalsIgnoreCase(Constants.Fetch_Accept_Sussess)){
		String[] stas = {Constants.Fetch_Accept_Sussess,Constants.Fetch_AcceptRedraw_Sussess};
		olist = projectExpertService.findMutiProjectExpertByPrjAndStatusOntime(prjid,stas);
		model.addAttribute("olist", olist);
		}
		if(fetchStatus.equalsIgnoreCase(Constants.Fetch_Accepted_Sussess)){
		String[] stas = {Constants.Fetch_Accepted_Sussess,Constants.Fetch_AcceptedRedraw_Sussess};
		olist = projectExpertService.findMutiProjectExpertByPrjAndStatusOntime(prjid,stas);
		model.addAttribute("olist", olist);
		}
        ...
        //保留抽取的结果到页面,若采纳需要使用到
        List<String> eclist =  Lists.newArrayList();
        List<String> pclist =  Lists.newArrayList();
        for(ProjectExpert ec : olist){
        	eclist.add(ec.getExpertExpertConfirm().getId());
        }
        for(ProjectInfo pc : plist){
        	pclist.add(pc.getId());
        }
        projectExpert.setResIds(StringUtils.join(eclist, ","));
        projectExpert.setPrjid(StringUtils.join(pclist, ","));
		model.addAttribute("projectExpert", projectExpert);

		return "modules/expfetch/fetchsearch/unitReceiveNote";
	}
	...
	public String receiveunitresult(ProjectExpert projectExpert, Model model, RedirectAttributes redirectAttributes, HttpServletRequest request,HttpServletResponse response) {
		...
		for(ProjectInfo prj:plist){
			for (String id : ids) {
				...
				projectExpertService.updateProjectExpertReviewDate(Constants.Fetch_Review_Sussess, prj.getId(), projectExpert.getReviewBegin(), projectExpert.getReviewEnd());
			}
			//项目ID重新拼
