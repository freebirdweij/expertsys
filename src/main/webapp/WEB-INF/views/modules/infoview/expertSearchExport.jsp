...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出用户数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/infoview/exportExpertSearch");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/infoview/expertsearchexp");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/infoview/projectsearchexp">查询项目</a></li>
		<li class="active"><a href="${ctx}/infoview/expertsearchexp">查询专家</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="expertConfirm" action="${ctx}/infoview/expertsearchexp" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="expertConfirm">
			<tr>
				...
				<td>
    				<a href="${ctx}/infoview/checkexpertfetchexp?expid=${expertConfirm.id}">查看抽取记录</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
