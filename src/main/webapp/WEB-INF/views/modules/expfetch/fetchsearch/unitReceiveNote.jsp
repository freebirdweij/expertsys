...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/fetchsearch/explist");
			$("#searchForm").submit();
	    	return false;
	    }
	    
		function rSubmit(){
			$("#inputForm").attr("action","${ctx}/expfetch/fetchsearch/export");
			$("#inputForm").submit();
	    	return false;
	    }
	    
		function rCancel(){
			$("#inputForm").attr("action","${ctx}/expfetch/fetchsearch/cancelunitresult");
			$("#inputForm").submit();
	    	return false;
	    }
	    
		function bCancel(){
			$("#inputForm").attr("action","${ctx}/expfetch/fetchsearch/reviewinglist");
			$("#inputForm").submit();
	    	return false;
	    }
...
<body>
	...
	<form:form id="inputForm" modelAttribute="projectExpert" action="${ctx}/expfetch/fetchsearch/directdrawunit" method="post" enctype="multipart/form-data" class="form-horizontal">
	...
	<table id="resultTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${olist}" var="projectExpert">
			<tr>
				<td><a href="${ctx}/expmanage/binfo?id=${projectExpert.expertExpertConfirm.id}">${projectExpert.expertExpertConfirm.expertInfo.name}</a></td>
				<td>${projectExpert.expertExpertConfirm.expertInfo.unit.name}</td>
				<td>${fns:getDictLabel(projectExpert.expertExpertConfirm.expertKind,'sys_specialkind_type','')}</td>
				<td>${fns:getDictLabel(projectExpert.expertExpertConfirm.expertSpecial,'sys_special_type','')}</td>
				<td>${fns:getDictLabel(projectExpert.expertExpertConfirm.expertInfo.technical,'sys_tech_type','')}</td>
				<td>${fns:getDictLabel(projectExpert.expertExpertConfirm.expertInfo.education,'sys_education_type','')}</td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
			<div class="controls">
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;抽取人:
						${projectExpert.createBy.name}
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;监督人:
						${projectExpert.supervise}
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;日期:
						${projectExpert.createDate}
			</div>
