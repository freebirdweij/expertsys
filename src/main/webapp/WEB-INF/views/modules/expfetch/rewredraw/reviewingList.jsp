...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/expfetch/rewredraw/reviewinglist");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/expfetch/rewredraw/reviewinglist">待评审项目</a></li>
		...
		<c:forEach items="${list}" var="projectInfo">
			<tr id="${projectInfo.id}" pId="${projectInfo.parent.id ne requestScope.projectInfo.id?projectInfo.parent.id:'0'}">
		        ...
		        <c:if test="${projectInfo.parent.id eq '0'}">
    				<a href="${ctx}/expfetch/rewredraw/unitmethod?prjid=${projectInfo.id}">进入补抽</a>
		        </c:if>
